...
public class AuthenticationService {
    ...
    private void sendValidationToken(User user) throws Exception {
        ...
        throw new RuntimeException("Both email and phone are missing or both are provided. One must be specified for OTP delivery."); // todo - make better exception
    }
    ...
    public AuthenticationResponse accountVerification(String username, String verificationCode) throws Exception {
        ...
        if (user.getDateOfBirth() == null) {
            throw new IllegalStateException("Enter your date of birth first to receive the verification code.");
        }
        if (token.getExpiresAt().isBefore(LocalDateTime.now())) {
            sendValidationToken(user);
            throw new IllegalArgumentException("Verification token expired! We've sent a new one to your email. Please check your inbox.");
        }
        if (token.getValidatedAt() != null) {
            throw new IllegalStateException("A new token has been sent! The previous one is already validated.");
        }
